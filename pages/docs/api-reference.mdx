...
> 
> **v3.2.0+**: 
> - In the original build, `cn` now defaults `twMerge` to `true` and uses `tailwind-merge` for automatic conflict resolution. Use `cx` if you need a lightweight utility without conflict resolution.
> - **Breaking change**: `cnBase` should be replaced with `cx`. The `cx` function provides the same functionality with a clearer name.
