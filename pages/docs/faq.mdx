...
### What's the difference between `cx` and `cn`? (v3.2.0+)

`cx` and `cn` are both utility functions for concatenating class names, but they serve different purposes:
...
- Uses `tailwind-merge` for automatic conflict resolution (v3.2.0+)
- Resolves conflicting Tailwind classes intelligently
- Defaults `twMerge` to `true` in the original build
- Use when you need automatic conflict resolution
