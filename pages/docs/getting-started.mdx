...
> **Note**: The lite build doesn't support the `twMerge` and `twMergeConfig` options. Choose this if bundle size is critical and you don't need automatic conflict resolution. Both `cn` and `cx` are available in both builds, with `cn` providing conflict resolution in the original build (v3.2.0+).
