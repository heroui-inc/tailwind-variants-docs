...
- **Two builds** — Default includes merge. `tailwind-variants/lite` skips the table, or lets you inject one you already have.
...
Split a component into named parts like `base`, `icon`, and `label`, then style each independently.
...
Build on existing recipes — one parent or several at once. Extend a base button to add an `iconOnly` variant, or compose shared focus and motion recipes, without duplicating styles.
