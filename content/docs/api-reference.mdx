...
type TVConfig = {
  ...
  debug?: boolean; // experimental; default entry, dev and browser only
};
```

`debug` logs how each invocation resolves; it is stripped from production builds. See [Debugging](/docs/debugging).

Lite config is `{ twMerge?: boolean | TwMergeFn }` only. `twMergeConfig` and `debug` are default-entry only. A function `twMerge` is invoked.
...
On the default build, `cn` returns a `string` directly. An empty class list yields `""`. Lite exports a curried `cn` that joins; prefer `createCN` when injecting a merger.
...
| `TVConfig` | Default-entry config: `twMerge`, `twMergeConfig`, `debug` |
