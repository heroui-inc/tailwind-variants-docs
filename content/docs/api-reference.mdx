...
// Usage - with conflict resolution (default build)
...
> - In the default build, `cn` resolves conflicting Tailwind classes. Use `cx` if you need a lightweight utility without conflict resolution.
...
**Added in v3.2.2** — Available on the **default build only** (`tailwind-variants`). Not exported from `/lite`.
...
**description:** Config object for conflict resolution. Prefer `{ extend, override }`. Compatible with common `tailwind-merge` config shapes. See [Conflict Resolution](/docs/conflict-resolution).
