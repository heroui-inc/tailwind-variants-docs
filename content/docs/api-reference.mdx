...
| Option | Description |
| --- | --- |
| `extend` | Merge parent recipes' base, slots, variants, defaults, compounds; non-empty arrays merge left to right (v3.4+) |
...
type TVConfig = {
  ...
  debug?: boolean; // experimental, v3.4+ — dev/browser only
};
```
...
cn(); // => "" (v3.4+ — never undefined)
```

Default build only. Like all utilities and `tv()` results, `cn` always returns a `string` — an empty class list yields `""` (v3.4+).
