...
Unknown or misspelled keys in `variants`, `compoundVariants`, and `defaultVariants` — and unknown slot names in slot-shaped class values — now fail at compile time. Previously they were silently ignored at runtime, so new errors after upgrading usually point at real typos:
