...
Since v3.4, unknown or misspelled keys in `variants`, `compoundVariants`, and `defaultVariants` — and unknown slot names in slot-shaped class values — fail at compile time instead of being silently ignored:

```ts
const button = tv({
  variants: { size: { sm: 'text-sm' } },
  // ✗ compile error — 'colour' is not a variant key
  defaultVariants: { colour: 'primary' }
});
```

If an upgrade surfaces new type errors here, they almost always point at real typos. See [Migration](/docs/migration#to-v340).
